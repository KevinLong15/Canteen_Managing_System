...
import com.zsct.store.adapter.DealListAdapter;
...
public class DealListActivity extends BaseActivity implements OnRefreshListener {
	...
	private ArrayList<Order> paydata = new ArrayList<Order>();
	private ArrayList<Order> donedata = new ArrayList<Order>();
	private ArrayList<Order> useddata = new ArrayList<Order>();
	private ArrayList<Order> backdata = new ArrayList<Order>();
	private ArrayList<Order> orderlistdata = new ArrayList<Order>();
	private int sectionindex = 0;
	private static final int pay_what = 1;
	private static final int done_what = 2;
	private static final int used_what = 3;
	private static final int back_what = 4;
	private String uid = "";
	private Runnable payRun, doneRun, usedRun,backRun;
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case pay_what:
				try {
					paydata = getListData(result);
					if (sectionindex == 0) {
						selectSecCheck(sectionindex);
						orderlistdata = paydata;
						Log.e("hjq", "size=" + orderlistdata.size());
						adapter = new DealListAdapter(DealListActivity.this,
								orderlistdata);
			...
			case used_what:
				try {
					useddata = getListData(result);
					if (sectionindex == 2) {
						selectSecCheck(sectionindex);
						orderlistdata = useddata;
						Log.e("hjq", "size=" + orderlistdata.size());
						adapter = new DealListAdapter(DealListActivity.this,
								orderlistdata);
						adapter.notifyDataSetChanged();
			...
			case back_what:
				try {
					backdata = getListData(result);
					if (sectionindex == 3) {
						selectSecCheck(sectionindex);
						orderlistdata = backdata;
						Log.e("hjq", "size=" + orderlistdata.size());
						adapter = new DealListAdapter(DealListActivity.this,
								orderlistdata);
						adapter.notifyDataSetChanged();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		orderList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View view,
				...
				Intent mIntent = new Intent(DealListActivity.this,
		...
		if (!result.contains(JsonUtil.CODE)) {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				orders.add(new Order(id, order_id, user_id, phone, store_id,
						...
						total_price, pay_type, pay_time,"0"));
			}
		}
		return orders;
	}
	...
	public void onClick(View v) {
		...
		case R.id.section1:
			...
			selectSecCheck(sectionindex);
			orderlistdata = paydata;
			adapter = new DealListAdapter(this, orderlistdata);
			adapter.notifyDataSetChanged();
			orderList.setAdapter(adapter);
			break;
		case R.id.section2:
	...
	private void initRun() {
		// TODO Auto-generated method stub
		uid = PreferenceUtil.getInstance(DealListActivity.this).getSid();
		payRun = new Runnable() {
			@Override
			public void run() {
				// TODO Auto-generated method stub
				String result = HttpUtil.post(HttpUtil.URL_GETGROUPORDER,
						new BasicNameValuePair(JsonUtil.STORE_ID, uid),
						new BasicNameValuePair(JsonUtil.STATUS, "submit"));
				Log.e("hjq",  HttpUtil.getURlStr(HttpUtil.URL_GETGROUPORDER,
						new BasicNameValuePair(JsonUtil.STORE_ID, uid),
						new BasicNameValuePair(JsonUtil.STATUS, "submit")));
		...
		backRun = new Runnable() {
			...
			public void run() {
				...
				String result = HttpUtil.post(HttpUtil.URL_GETGROUPORDER,
						new BasicNameValuePair(JsonUtil.STORE_ID, uid),
						new BasicNameValuePair(JsonUtil.STATUS, "drawback"));
				Message msg = new Message();
				msg.what = back_what;
				msg.obj = result;
				mHandler.sendMessage(msg);
		...
		ThreadPoolManager.getInstance().addTask(payRun);
		ThreadPoolManager.getInstance().addTask(doneRun);
		ThreadPoolManager.getInstance().addTask(usedRun);
		ThreadPoolManager.getInstance().addTask(backRun);
	}
